...
public class CategoryPage extends Page {
	...
	private void printExistingCategory(){
		if(term == null){return;}
		ArrayList<Category> category = term.getCategories();
		if(category != null && category.size() > 0){
			...
			for (Category bg : category) {
				categorySummary += "<tr>";
				categorySummary += "<td>"+bg.name+"</td><td>"+Double.toString(bg.getAmountSpent())+"</td><td>"+Double.toString(bg.getAmountRemaining())+"</td>";
				categorySummary += "</tr>";
			}
			categorySummary += "</tbody>";
			categorySummary += "</table>";
